...
import (
	"context"
	"module-calendar/event/repository/postgres"
	"net/http"
	"os"
	"os/signal"
	"syscall"
...
func main() {
	...
	db, err := postgres.NewPostgresDB(config.DBPath)
	if err != nil {
		panic(err)
	}
	store := postgres.NewPostgresEventRepository(db)
